...
function RealizeMeterAndBoosts( kControl:table, kData:table )
	...
	if kData.Boostable then
		--DA_Boost----------------------------------------
		local DA_BoostString = kData.TriggerDesc
		local Is_DA_BoostType = false
		if kData.TechType ~= nil and GameInfo.DA_Boosts[kData.TechType] ~= nil then
			Is_DA_BoostType = true
			DA_BoostString = GameInfo.DA_Boosts[kData.TechType].Text
		elseif kData.CivicType ~= nil and GameInfo.DA_Boosts[kData.CivicType] ~= nil then
			Is_DA_BoostType = true
			DA_BoostString = GameInfo.DA_Boosts[kData.CivicType].Text
		end
...
function RealizeTurnsLeft( kControl:table, kData:table)
	...
	elseif kData ~= nil and kData.CivicType ~= nil and GameInfo.DA_Boosts[kData.CivicType] ~= nil then
		local pPlayer = Players[Game.GetLocalPlayer()]
...
-- ===========================================================================
function RealizeCurrentResearch( playerID:number, kData:table, kControl:table, cachedModifiers:table)
	...
	if kData ~= nil then
		...
		--给科技加总督点好麻烦啊
		-- local TechModifiers:table = cachedModifiers[kData.TechType];
		-- if ( TechModifiers ) then
		-- 	for _,tModifier in ipairs(TechModifiers) do
		...
		numUnlockables = numUnlockables + PopulateUnlockablesForTech( playerID, kData.ID, techUnlockIM, nil);
...
function TechAndCivicSupport_BuildCivicModifierCache()
	...
	for tTechMod:table in GameInfo.TechnologyModifiers() do
		local tTechCache :table = tCache[tTechMod.TechnologyType];
		if ( not tTechCache ) then
			tTechCache = {};
			tCache[tTechMod.TechnologyType] = tTechCache;
		end
		
		local tModInfo :table = tModCache[tTechMod.ModifierId];
		assert( tModInfo );
		table.insert( tTechCache, tModInfo );
	end
