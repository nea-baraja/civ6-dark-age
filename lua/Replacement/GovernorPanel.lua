...
function Refresh()
	--for kGovernorDef in GameInfo.Governors() do
		--local governorId:number = kGovernorDef.Index;
		--LuaEvents.GovernorAssignmentChooser_RequestAssignment( governorId );
	--end
--LuaEvents.GovernorAssignmentChooser_RequestAssignment( 1 ,0,Players[Game.GetLocalPlayer()]:GetCities():GetCapitalCity():GetID());
	
	--local kParameters:table = {};
	--kParameters[PlayerOperations.PARAM_GOVERNOR_TYPE] = 5;
	...
	Controls.GovernorTitlesAvailable:SetText(Locale.Lookup("LOC_GOVERNOR_GOVERNOR_TITLES_AVAILABLE", governorPointsObtained - governorPointsSpent - hidedGovernors));
	Controls.GovernorTitlesSpent:SetText(Locale.Lookup("LOC_GOVERNOR_GOVERNOR_TITLES_SPENT", governorPointsSpent));
	Controls.GovernorTitlesHide:SetText(Locale.Lookup("LOC_GOVERNOR_GOVERNOR_TITLES_HIDE", policyGovernors));
	...
	for _, pAppointedGovernor:table in ipairs(tGovernorList) do
		...
		local bCanPromoteGovernor:boolean = playerGovernors:CanPromoteGovernor(kGovernorDef.Hash) and ((governorPointsObtained - governorPointsSpent - hidedGovernors) > 0);
	...
	for i, pAppointedGovernor:table in ipairs(tGovernorList) do
		...
		if (not IsCannotAssign(kGovernorDef) and GovernorUnlocked(kGovernorDef.GovernorType)) then
...
function GovernorUnlocked(eGovernorType)
	local pPlayer = Players[Game.GetLocalPlayer()]
	if (pPlayer == nil) then
		return
	end
	...
	local eStateWorkforceType = GameInfo.Civics['CIVIC_STATE_WORKFORCE'].Index
	
	if(eGovernorType == 'GOVERNOR_THE_BUILDER' and  (not pCulture:HasCivic(eStateWorkforceType))) then
		--return false
		return true
	end
	return true
