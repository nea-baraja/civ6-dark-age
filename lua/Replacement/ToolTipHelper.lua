...
ToolTipHelper.GetBuildingToolTip = function(buildingHash, playerId, city)
    ...
    for row in GameInfo.CitizenBonus() do
    	if(row.ItemType == buildingType) then
    		if row.Amount ~= 0 then
    			if row.BonusType:match "^GREAT_PERSON_CLASS_" then
    				local gpClass = GameInfo.GreatPersonClasses[row.BonusType];
					if(gpClass) then
						local greatPersonClassName = gpClass.Name;
						local greatPersonClassIconString = gpClass.IconString;
						table.insert(citizen_yields, Locale.Lookup("LOC_TYPE_TRAIT_GREAT_PERSON_POINTS", row.Amount, greatPersonClassIconString, greatPersonClassName));
					end
				else
    				table.insert(citizen_yields, "[ICON_Bullet] " .. Locale.Lookup("LOC_"..row.BonusType, row.Amount));
    			end
    		else
    			table.insert(citizen_yields, "[ICON_Bullet] " .. Locale.Lookup("LOC_"..row.BonusType));
    		end
    	end
    end
...
ToolTipHelper.GetCivicToolTip = function(civicType, playerId)
	...
	-- Darkage changes begin
	if GameInfo.DA_Boosts[civicType] ~= nil then
		local pPlayer = Players[playerId]
		local CivicBoostTriggerByTask = (pPlayer:GetProperty("DA_Boost_"..civicType) or {})['ByTask'] or 0
		if CivicBoostTriggerByTask ~= 0 then
			table.insert(toolTipLines, Locale.Lookup("LOC_DA_BOOST_ADD_BYTASK", Round(CivicBoostTriggerByTask * 100)))
		end
		local CivicBoostTriggerByOther = (pPlayer:GetProperty("DA_Boost_"..civicType) or {})['ByOther'] or 0
		if CivicBoostTriggerByOther ~= 0 then
			table.insert(toolTipLines, Locale.Lookup("LOC_DA_BOOST_ADD_BYOTHER", CivicBoostTriggerByOther * 100))
		end
...
ToolTipHelper.GetDistrictToolTip = function(districtType)
    ...
    --Darkage changes begin
    for row in GameInfo.CitizenBonus() do
    	if(row.ItemType == districtType) then
    		if row.Amount ~= 0 then
    			table.insert(citizen_yields, "[ICON_Bullet] " .. Locale.Lookup("LOC_"..row.BonusType, row.Amount));
    		else
    			table.insert(citizen_yields, "[ICON_Bullet] " .. Locale.Lookup("LOC_"..row.BonusType));
...
ToolTipHelper.GetTechnologyToolTip = function(techType, playerId)
	...
	-- Darkage changes begin
	if GameInfo.DA_Boosts[techType] ~= nil then
		local pPlayer = Players[playerId]
		local TechBoostTriggerByTask = (pPlayer:GetProperty("DA_Boost_"..techType) or {})['ByTask'] or 0
		if TechBoostTriggerByTask ~= 0 then
			table.insert(toolTipLines, Locale.Lookup("LOC_DA_BOOST_ADD_BYTASK",Round(TechBoostTriggerByTask * 100)))
		end
		local TechBoostTriggerByOther = (pPlayer:GetProperty("DA_Boost_"..techType) or {})['ByOther'] or 0
		if TechBoostTriggerByOther ~= 0 then
			table.insert(toolTipLines, Locale.Lookup("LOC_DA_BOOST_ADD_BYOTHER",TechBoostTriggerByOther * 100))
		end
