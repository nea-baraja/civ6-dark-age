...
g_victoryData = {
	...
	VICTORY_HARMONY_IN_DIVERSTY = {
		GetText = function(p) return "已经激活{1_count}伟人" end,
		GetScore = function(p) return p:GetProperty('NumberOfUsedGreatPeople') or 0 end,
		AdditionalSummary = function(p) return GetDominationVictoryAdditionalSummary(p) end
	}
...
function ViewOverall()
	...
	if(Game.IsVictoryEnabled("VICTORY_RELIGIOUS")) then
		...
		PopulateOverallInstance(m_OverallIM:GetInstance(), "VICTORY_HARMONY_IN_DIVERSTY", "HARMONY_IN_DIVERSTY");
			...
			if(progress ~= nil) then
				...
				for i, playerID in ipairs(team) do
					if IsAliveAndMajor(playerID) then
						...
						if victoryType == 'VICTORY_HARMONY_IN_DIVERSTY' then
							playerData[playerID].progress = primaryScore / 10;
						end
				...
				for i, pData in pairs(playerData) do
					if pData.progress and pData.progress > highestProgress then
						highestProgress = pData.progress;
					end
				end

				if highestProgress ~= 0 then 
