...
function GetDistrictYieldText(district)
    ...
    for yield in GameInfo.Yields() do
        local yieldAmount = district:GetYield(yield.Index) - district:GetAdjacencyYield(yield.Index);
...
function GetDistrictAdjacencyYieldText(district)
    local yieldText = "";
    for yield in GameInfo.Yields() do
        local yieldAmount = district:GetAdjacencyYield(yield.Index);
        if yieldAmount ~= 0 then
            yieldText = yieldText .. GetYieldString( yield.YieldType, yieldAmount );
        end
    end
...
function GetCityData( pCity:table )
    ...
    -- --DA Happiness
    -- function GetHappinessYieldForDA_( yieldEnum:number , amenity:number)
    ...
    -- data.HappinessYieldForDA[YieldTypes.CULTURE]   = GetHappinessYieldForDA_(YieldTypes.CULTURE, data.AmenitiesNetAmount);
    -- data.HappinessYieldForDA[YieldTypes.FAITH]     = GetHappinessYieldForDA_(YieldTypes.FAITH, data.AmenitiesNetAmount);
    -- data.HappinessYieldForDA[YieldTypes.GOLD]      = GetHappinessYieldForDA_(YieldTypes.GOLD, data.AmenitiesNetAmount);
    -- data.HappinessYieldForDA[YieldTypes.PRODUCTION]= GetHappinessYieldForDA_(YieldTypes.PRODUCTION, data.AmenitiesNetAmount);
    -- data.HappinessYieldForDA[YieldTypes.SCIENCE]   = GetHappinessYieldForDA_(YieldTypes.SCIENCE, data.AmenitiesNetAmount);
    ...
    for i, district in pCityDistricts:Members() do
        ...
        local districtTable :table	= { 
            ...
            YieldAdjacencyBonus  = GetDistrictAdjacencyYieldText( district ),
        ...
        data.DistrictCitizens = data.DistrictCitizens + districtTable.Citizens;
