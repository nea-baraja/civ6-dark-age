...
-- ARISTOS
local isBRSActive:boolean = Modding.IsModActive("6f2888d4-79dc-415f-a8ff-f9d81d7afb53"); -- Better Report Screen (Infixo)
...
local m_isAllowAnythingInWildcardSlot :boolean = false;  -- Currently engine doesn't allow because on confirmation Culture::CanSlotPolicy() first checks !IsPolicyActive( ePolicy ), which fails.
...
-- ===========================================================================
-- 皮凯: 现在构建双倍政策卡UI和lua程序 
local DA_Copy_ActiveCard = {} -- 记录当前复制政策卡的激活状态以及对应卡槽ID
local DA_Copy_CardHiddenSlots = {} -- 记录当前激活复制政策卡数量和应该正常显示非复制政策卡的卡槽ID

function RemoveActiveCopyPolicyOriginalPolicy(originalPolicy:string)
  local DACopyCard = DA_Copy_ActiveCard[originalPolicy]
  if DACopyCard.Active and DACopyCard.Card ~= -1 then
    RemoveActivePolicyAtSlotIndex(DACopyCard.Card)
    DA_Copy_ActiveCard[originalPolicy] = {Active = false, Card = -1}
  end
end
-- ===========================================================================
--  FUNCTIONS
...
function GetFreeSlotCountForRow( nRowIndex:number )
  ...
  for _,tSlotData in ipairs(m_ActivePolicyRows[nRowIndex].SlotArray) do
    if ( tSlotData.GC_PolicyType == EMPTY_POLICY_TYPE and tSlotData.UI_RowIndex ~= ROW_INDEX.MILILOCK) then
      nFree = nFree + 1;
    end
  end
  return nFree;
...
function RealizePolicyCard( cardInstance:table, policyType:string )
  ...
  -- ARISTOS
  -- If Better Report Screen mod is active show the effects of enabling this policy in the description
  -- and a new container at the bottom of the card.
  if(isBRSActive) then
  ...
  for _,policyType in pairs(m_kPolicyCatalogOrder) do
    ...
    then
      ...
      then
        ...
        if(isCivilopediaAvailable) then
          cardInstance.Button:RegisterCallback( Mouse.eMClick, function() LuaEvents.OpenCivilopedia(policyType); end);
        end
        ...
        local notCopyPolicy = policyType:sub(1,8) ~= "DA_COPY_"
        cardInstance.Content:SetShow(notCopyPolicy);
        local tempTable = notCopyPolicy and {} or nil
        if tempTable then
...
function RealizeActivePoliciesRows()
  ...
  for nRowIndex,tRow in ipairs(m_ActivePolicyRows) do
    ...
    DA_Copy_CardHiddenSlots[nRowIndex] = { HiddenSlotNum = 0, ShowSlots = {}}
    for nRowSlotIndex,tSlotData in ipairs(tRow.SlotArray) do      
      ...
      if ( policyType ~= EMPTY_POLICY_TYPE ) then
        ...
        local fnRemoveCard = function( )
            ...
            RemoveActiveCopyPolicyOriginalPolicy(policyType)
            RealizePolicyCatalog();
            RealizeActivePoliciesRows();
          end
        ...
        if(isCivilopediaAvailable) then
          cardInst.Button:RegisterCallback( Mouse.eMClick, function() LuaEvents.OpenCivilopedia(policyType); end);
        end
        ...
        local notCopyPolicy = policyType:sub(1,8) ~= "DA_COPY_"
        cardInst.Content:SetShow(notCopyPolicy);
        local tempTable = notCopyPolicy and {} or nil
        local DA_Copy_Card = DA_Copy_ActiveCard[policyType]
        if tempTable then
          tempTable[KEY_POLICY_TYPE] = "DA_COPY_" ..policyType
  ...
  lockCardInst.TypeIcon:SetAlpha(0.5);
  ...
  lockCardInst.LockPolicyLabel:SetText('消耗1 [icon_governor] 总督头衔增加一个军事政策卡槽');      
  lockCardInst.Content:RegisterMouseEnterCallback(function() lockCardInst.TypeIcon:SetAlpha(1); end);
  lockCardInst.Content:RegisterMouseExitCallback(function() lockCardInst.TypeIcon:SetAlpha(0.5); end);
 ]]


  --政体任务  已废弃
  local pPlayer = Players[m_ePlayer];
  local govID = pPlayer:GetCulture():GetCurrentGovernment()
  if govID ~= nil and govID ~= -1 then 
    ...
    if govType == 'GOVERNMENT_TRIBE_UNITY' then
      ...
      if iBarb < 2 then
        ...
        lockCardInst[KEY_DRAG_TARGET_CONTROL] = lockCardInst.Content;
        lockCardInst[KEY_LIFTABLE_CONTROL]    = lockCardInst.LiftableContainer;
        lockCardInst.LockPolicyLabel:SetText('[ICON_Exclamation]不可用[NEWLINE][NEWLINE]部落联盟执政时，累计清理2个蛮族哨站后解锁。已清理'..iBarb..'个。');      
      end
      if iGoody < 5 then
        local lockCardInst = {};
...
function GetEmptyPolicySlotTexture(typeIndex : number)
  if typeIndex == 4 then return 'Governments_WildcardCard_New_Empty.dds' end
...
function SwitchTab( tabFrom, tabTo, bForce )
  ...
  Controls.StackEconomic:SetConsumeMouseButton( isInputConsumed );
  Controls.StackMilitary:SetConsumeMouseButton( isInputConsumed );
  Controls.StackWildcard:SetConsumeMouseButton( isInputConsumed );
  Controls.StackDiplomatic:SetConsumeMouseButton( isInputConsumed );
  Controls.PolicyInputShield:SetConsumeMouseButton( false );
  ...
  ButtonAdjustExchangePolicy(46)
  --Controls.MyGovernment:SetHide( false );
end
-- ===========================================================================
-- 皮凯(PK)：构造一个示范存放总督兑换政策卡槽情况的表，或许需要数据库设定对应表以便提高扩展性和兼容性
-- 注意m_GovernorExchangePolicy这部分应当成为存入存档的内容，这部分和具体的给予政策卡槽程序我未完成，我忘记DA的存表接口，以及不清楚DA的政策的卡槽修改器具体ID
-- 额外未完成需要给玩家存一个数值，统计在政策这里消耗的总督数量，以便总督UI那里使用

local m_GovernorExchangePolicy = Players[Game.GetLocalPlayer()]:GetProperty('PROP_EXCHANGE_POLICY');
...
-- 皮凯(PK)：这个表用来精简重复的代码
local PKpolicyTypes = {"Military", "Economic", "Diplomatic", "Wildcard"}
-- 皮凯(PK)：用来调整按钮位置
function ButtonAdjustExchangePolicy(X:number)
  for _, policyType in ipairs(PKpolicyTypes) do
    Controls["GovernorExchange"..policyType.."Button"]:SetOffsetX(X)
  end
end
-- 皮凯(PK)：获取当前玩家总督点
function GetGovernorNum()
  local pPlayer = Players[Game.GetLocalPlayer()];
  if pPlayer == nil then return end
  local playerGovernors = pPlayer:GetGovernors();
  local hidedGovernors = pPlayer:GetProperty('PROP_HIDE_GOVERNOR') or 0;
  return playerGovernors:GetGovernorPoints() - playerGovernors:GetGovernorPointsSpent() - hidedGovernors;
end
-- 皮凯(PK)：对应按钮禁用状态
function GEProhibitButtons(policyType)
  -- 此时该类型政策卡槽以达到兑换上限无法兑换
  Controls["GovernorExchange"..policyType.."Button"]:SetDisabled(true) -- 禁用状态
  Controls[policyType.."Remind"]:SetText("[ICON_Checkmark]")
  Controls[policyType.."Remind"]:SetShow(true)
  Controls[policyType.."GovernorCount"]:SetText("-")
  Controls["GovernorExchange"..policyType.."Button"]:SetToolTipString("以完成所有政策卡槽兑换？文本没想好")
end
-- 皮凯(PK)：注册对应按钮程序
function GERegisterButtons()
  -- 通用按钮点击事件处理函数
  local function OnGEButtonClick(policyType)
    local GovernorNum = GetGovernorNum();
    UI.PlaySound("Main_Menu_Mouse_Over")
    --if GovernorNum == nil then return end
    -- 对应政策的卡槽给予函数待补充
    local pPlayer = Players[Game.GetLocalPlayer()];
    local hidedGovernors = pPlayer:GetProperty('PROP_HIDE_GOVERNOR') or 0;
    local policyGovernors = pPlayer:GetProperty('PROP_POLICY_GOVERNOR') or 0;
    local function BuyPolicySlot()
      m_GovernorExchangePolicy[policyType] = m_GovernorExchangePolicy[policyType] + 1;
      GameEvents.SetPlayerProperty.Call(Game.GetLocalPlayer(), 'PROP_EXCHANGE_POLICY', m_GovernorExchangePolicy);
      GameEvents.SetPlayerProperty.Call(Game.GetLocalPlayer(), 'PROP_HIDE_GOVERNOR', hidedGovernors + 1);
      GameEvents.SetPlayerProperty.Call(Game.GetLocalPlayer(), 'PROP_POLICY_GOVERNOR', policyGovernors + 1);      
      GameEvents.PlayerAttachModifierByID.Call(Game.GetLocalPlayer(), 'ADD_'..policyType);
      LuaEvents.GovernorPointsHided(0, 1);
    end

    local pPopupDialog :table = PopupDialogInGame:new("BuyPolicySlot"); -- unique identifier
    pPopupDialog:AddTitle(Locale.Lookup('LOC_BUY_POLICY_SLOT'));
    pPopupDialog:AddText(Locale.Lookup('LOC_'..string.upper(policyType)..'_INFO'));
    pPopupDialog:AddConfirmButton(Locale.Lookup("LOC_YES"), BuyPolicySlot);
  ...
  for _, policyType in ipairs(PKpolicyTypes) do
    Controls["GovernorExchange"..policyType.."Button"]:RegisterCallback(Mouse.eLClick, function() OnGEButtonClick(policyType) end)
  end
end
-- 皮凯(PK)：打开政策UI刷新总督兑换UI
function RefreshGE()
  local GovernorNum = GetGovernorNum()
  ...
  for _, policyType in ipairs(PKpolicyTypes) do
    ...
    if GovernorNum > 0 then
      Controls["GovernorExchange"..policyType.."Button"]:SetDisabled(false) 
      Controls[policyType.."GovernorCount"]:SetText(iGovernorNum)
      Controls[policyType.."Remind"]:SetShow(true) -- 达到要求时会提醒可以兑换                                                -- 这里文本没设想好，随便设想的
      Controls["GovernorExchange"..policyType.."Button"]:SetToolTipString(Locale.Lookup("LOC_"..capitalPolicy.."_ADD", iGovernorNum)) -- 兑换新的"capitalPolicy"政策卡槽需要{1_iGoverniorNum}个总督头衔,目前对比总数{1_iGoverniorNum} / {2_GoverniorNum}
    else
      Controls["GovernorExchange"..policyType.."Button"]:SetDisabled(true) -- 禁用状态
      Controls[policyType.."GovernorCount"]:SetText(iGovernorNum)
      Controls[policyType.."Remind"]:SetShow(false);
      Controls["GovernorExchange"..policyType.."Button"]:SetToolTipString(Locale.Lookup("LOC_"..capitalPolicy.."_ADD", iGovernorNum));
    end
  end
end

function lagRefresh()
  Controls.RefreshTimer:SetToBeginning();
  Controls.RefreshTimer:Stop();
  RefreshAllData();
end
...
function OnDropFromCatalog( dragStruct:table, cardInstance:table )
  ...
  if nTargetRow ~= -1 then
    ...
    if ( tTargetCardInst ~= nil ) then
      local targetPolicySlot = tTargetCardInst[KEY_POLICY_SLOT]
      local targetPolicyType = tTargetCardInst.PolicyType
...
-- ===========================================================================
function AddToNextAvailRow( cardInstance:table, copiedPolicyType:string )
  local policyType  :string = cardInstance[KEY_POLICY_TYPE];
  ...
  local TryToAddToRow = function( nRowIndex:number, strSlotType:string, strPolicyType:string )
      if IsSlotTypeLegalInRow( nRowIndex, strSlotType ) then
        ...
        if ( nSlot ~= -1 ) then
          ...
          if copiedPolicyType then DA_Copy_ActiveCard[copiedPolicyType]= {Active = true, Card = nSlot} end
          return true;
        end
      end
      return false;
...
function OnDropFromRow( dragStruct:table, cardInstance:table )  
  ...
  if nTargetRow == -1 then
    ...
    -- 官方bug，这里不会运行，或者说官方这里拖离卡槽卡时如果拖动目的地也是一个同类型卡，卡槽替换为这个卡的功能失败
...
--[[
  local tSlotData_ :table = {
    -- Static members
    UI_RowIndex   = ROW_INDEX.MILILOCK,
    GC_SlotIndex  = nPolicySlots,

    -- Dynamic members
    GC_PolicyType = EMPTY_POLICY_TYPE
    };
  table.insert( m_ActivePolicyRows[ROW_INDEX.MILITARY].SlotArray, tSlotData_ );
  m_ActivePoliciesBySlot[nPolicySlots] = tSlotData_;
]]
...
function Initialize()
  ...
  Controls.RowAnim:RegisterEndCallback( function()
      if Controls.RowAnim:IsReversing() then
        --Controls.CategoryIconsLeft:SetOffsetX(Controls.CategoryIconsLeft:GetOffsetX() - 16);
        Controls.CategoryIconsLeft:SetHide( true );
        ButtonAdjustExchangePolicy(28)
  ...
  GERegisterButtons()
  Controls.RefreshTimer:RegisterEndCallback(lagRefresh);
  -- Gamecore EVENTS
